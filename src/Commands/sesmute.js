...
module.exports = {
    name: "ses-mute",
    aliases: [],
    description: 'kullanıcıyı sesli kanallardan uzaklaştırır.',
    async execute(message, args) {
        if(message.member.roles.cache.has(roller['sorumlu-rolleri']['chat-sorumlusu']) || message.member.hasPermission('ADMINISTRATOR')){
            ...
            let atıldı = üye.voice.serverMute(true).catch(err => console.log(err.message));
            
            if(atıldı){
                let s = new MessageEmbed()
                .setDescription(`${üye} adlı kullanıcı başarılı bir şekilde ses mute atıldı. `)
                .setFooter(message.guild.name)
                message.channel.send(s).catch(err => console.log(err.message));
                
                setTimeout(async() => {
                    await üye.voice.serverMute(true).catch(err => console.log(err.message));
                    let s = new MessageEmbed()
                    .setDescription(`${üye} adlı kullanıcı başarılı bir şekilde ses mutesi açıldı. `)
                    .setFooter(message.guild.name)
                    message.channel.send(s).catch(err => console.log(err.message));
                },ms(süre));
